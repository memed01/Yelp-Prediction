...
evallm<-postResample(round(predict(reg,testData)),round(testData$stars))

#evalAcc<-accuracy(round(predict(reg,testData)),round(testData$stars))

```
The Peirces skill score (PSS) is `r pss1`, the global error rate (TG) is `r tg1`, the root mean squared error (RMSE) is `r evallm[1]` and this is our final model.
